...
export default function MiniAppsModal({ isOpen, onClose, miniApps, onSelectMiniApp }: MiniAppsModalProps) {
  ...
  const categories = ['All', ...Array.from(new Set(miniApps.map(app => app.category)))];
  ...
  const getIcon = (iconName: string) => {
    ...
    return IconComponent ? <IconComponent size={24} /> : <Icons.Square size={24} />;
  };
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          {filteredApps.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No MiniApps found matching your criteria.</p>
            </div>
          ) : (
